...
void printMessageCenter(const char* message)
{
    ...
    len = (78 - strlen(message))/2; //strlen-The strlen() function calculates the length of a given string
    printf("\t\t\t");
...
int isNameValid(const char *name)
{
    ...
    {
        if(!(isalpha(name[index])) && (name[index] != '\n') && (name[index] != ' '))//alpha-check whether alphaber or not
        {
            validName = 0;
            break;
        }
...
int isValidMobNumber(const char *name)
{
    ...
    for(index =0; index <len ; ++index)
    {
        if(!(isdigit(name[index])) && (name[index] != '\n') && (name[index] != ' '))//isdigit(c) is a function in C which can be used to check if the passed character is a digit
        {
            validName = 0;
            break;
        }
